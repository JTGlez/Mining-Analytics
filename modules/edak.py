...
# Uso API propia, idealmente el usuario debe introducir su propia clave API.
api = KaggleApi()
api.authenticate()
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    ...
                    type = 'text',
                    value = '',
                    placeholder='Buscar dataset en Kaggle'
                    ),
...
def update_output(list_of_contents, n_clicks, list_of_names, list_of_dates, dataset_ref):
    ...
    if ctx.triggered[0]['prop_id'] == 'upload-data.contents':
        if list_of_contents is not None:
            ...
            return children
    elif ctx.triggered[0]['prop_id'] == 'download-dataset-button.n_clicks':
        if n_clicks and dataset_ref:
            api.dataset_download_files(dataset_ref, unzip=True)
            df = load_downloaded_dataset(dataset_ref)
            if df is not None:
                return eda(df, dataset_ref)
            else:
                return "No se pudo cargar el archivo CSV del conjunto de datos descargado."
        return None

def search_datasets(query):
    search_results = api.dataset_list(search=query)
    options = [{'label': result.title, 'value': result.ref} for result in search_results]
    return options
...
def update_dataset_options(search_query):
    if search_query:
        return search_datasets(search_query)
    else:
        return []
