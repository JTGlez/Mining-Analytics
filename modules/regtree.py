...
def regtree_card():
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(
                id="intro",
                children="Explicación arbol regresor"
                ,
            ),
...
@callback(Output('output-data-upload-regtree', 'children'),
              [Input('upload-data-regtree', 'contents'),
               Input('upload-data-static-regtree', 'value')],
...
def update_output(list_of_contents, selected_file, list_of_names, list_of_dates):
    ...
    if ctx.triggered[0]['prop_id'] == 'upload-data-regtree.contents':
        if list_of_contents is not None:
            ...
            return children
    elif ctx.triggered[0]['prop_id'] == 'upload-data-static-regtree.value':
        df = pd.read_csv(selected_file)
        return regtree(df, selected_file)